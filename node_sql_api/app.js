...
async function addAttendancea(uid, student_id, status, date, time,date_time) {
    return new Promise((resolve, reject) => {
        const sql = `INSERT INTO attendance (uid, student_id, status, Date, Time, Date_time) 
                     VALUES (?, ?, ?, ?, ?,?)`;
        db.query(sql, [uid, student_id, status, date, time,date_time], (err, result) => {
            if (err) {
    ...
    if (uid) {
        try {
            ...
            if (student) {
                ...
                await addAttendancea(uid, student_id, status, date, time,date_time);
